...
refinamento adicional; no cenário de e-commerce, em que cada conjunto de dados já vem de uma fonte
...
ao seu conjunto de dados. Como alternativa equivalente, o modo combinado reúne os mesmos eventos em
...
governada por onze opções, que precisam ser digitadas corretamente a cada execução e cujos
...
de arquitetura: os módulos que montam o comando, resolvem o executável e traduzem a saída não
dependem da biblioteca gráfica, podendo ser testados sem instanciar uma aplicação de janelas.

A janela divide-se em duas regiões, separadas por um divisor ajustável, como mostra a
Figura~\ref{fig:gui-ocioso}. Na metade superior estão três cartões de formulário: os dois
arquivos de configuração, selecionados por diálogos de arquivo; as opções de execução, com
um controle para cada \textit{flag} --- os SGBDs de destino, a estratégia de escrita e o
modelo de execução discutidos na Seção~\ref{sec:modelos-execucao}, os modificadores de
simulação, criação de esquema e \textit{benchmark}, o nível de \textit{log} e a exibição de
dados; e a tabela de sobrescrita de caminhos das fontes CSV, cujas linhas nascem da escolha
de arquivos --- um seletor de arquivos com seleção múltipla, ou uma pasta inteira, da qual
todos os arquivos com extensão \texttt{.csv} são anexados de uma vez. O nome de cada fonte é
deduzido do arquivo escolhido comparando-o com o bloco \texttt{sources} da configuração, de
modo que a sobrescrita gerada se refira a uma fonte que de fato existe. Na metade inferior
está o console integrado, que exibe o comando montado e, abaixo dele, a saída da execução.
Antes de
executar, a interface faz uma \textbf{validação local} restrita ao que pode conferir sem
abrir os arquivos --- existência dos caminhos, nomes de fonte não vazios e não repetidos ---
e mantém o botão de execução desabilitado enquanto houver pendências, reservando a validação
semântica ao \textit{JSON Schema}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{images/figure12-gui-ocioso}
  \caption{Interface gráfica no estado ocioso: formulário preenchido e comando montado em
...
produzia alteração visível no texto, e o clique parecia ter sido ignorado. Escrever a opção
por extenso custa alguns caracteres e devolve ao comando a propriedade que justifica exibi-lo
--- a de que cada controle do formulário tem um correspondente literal na linha de comando.
Um \textbf{modo de edição} permite editar o texto diretamente;
ao entrá-lo, o formulário é \textbf{bloqueado}, sinalizando que o texto passou a ser a fonte
da verdade. Essa decisão é intencional e tem uma razão precisa: \textbf{não existe analisador
de comando} na ferramenta. O caminho do formulário para o texto é de mão única, e manter os
dois editáveis ao mesmo tempo exigiria reconstruir o estado do formulário a partir de uma
cadeia de caracteres arbitrária --- precisamente o tipo de acoplamento que a interface se
propõe a evitar. Ao sair do modo de edição, o texto digitado é descartado mediante
confirmação e o comando é remontado a partir dos controles.

Durante a execução, o formulário permanece desabilitado, o botão principal passa a oferecer
a interrupção do processo e a saída da CLI chega ao console de forma incremental, como ilustra
a Figura~\ref{fig:gui-executando}. Ao término, a barra de estado informa o desfecho de maneira
explícita: a Figura~\ref{fig:gui-sucesso} mostra uma execução concluída, com o tempo total e o
caminho do arquivo de \textit{log} da sessão, e a Figura~\ref{fig:gui-erro} mostra uma execução
malsucedida, com a mensagem de erro preservada no console e o código de saída informado. Esse
último ponto merece registro: como a interface executa a CLI real, um erro de configuração ou
de conexão produz exatamente a mesma mensagem que produziria no terminal, sem tradução
intermediária que pudesse mascará-lo.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{images/figure13-gui-executando}
  \caption{Execução em andamento: formulário bloqueado, botão de interrupção e saída da CLI
  chegando ao console em tempo real.}
...
\begin{figure}[H]
  ...
  \caption{Execução malsucedida: a mensagem de erro da CLI é preservada no console e o código
  de saída é informado na barra de estado.}
  \label{fig:gui-erro}
  \par\vspace{4pt}
  {\footnotesize Fonte: Elaborado pelo autor (2026).}
\end{figure}

Duas limitações devem ser registradas. A primeira é de apresentação: a saída da CLI é
colorida por meio de sequências de escape ANSI quando escrita em um terminal, mas, no Windows,
a biblioteca de formatação emprega chamadas próprias do console do sistema operacional, que
não produzem efeito quando a saída é redirecionada para um canal de comunicação entre
processos. O console integrado, portanto, apresenta a saída em tom único nessa plataforma,
preservando integralmente o seu conteúdo, a sua estrutura e as barras de progresso, mas não
as suas cores. A segunda é de escopo: a interface \textbf{monta e executa} o comando, mas não
\textbf{edita} os arquivos JSON de configuração, que continuam a ser mantidos em um editor de
texto externo. Ampliar a interface para editar a configuração de forma assistida é uma das
atividades futuras discutidas no Capítulo~\ref{cap:futuros}.
