...
Assim sendo, este trabalho desenvolve a ferramenta \textit{Polyglot Import
CSV}, que importa dados armazenados em CSV para SGBDs relacionais e NoSQL. A configuração
da ferramenta baseia-se em dois arquivos JSON: um de importação, mapeando entidades e
...
\begin{itemize}
  ...
  \item Avaliar o desempenho da ferramenta para diferentes volumes e modelos de dados
        destino;
  \item Disponibilizar a ferramenta de forma acessível ao usuário: por meio de uma interface
        de linha de comando (CLI) e de uma interface gráfica (GUI) que monta e executa o
        comando de importação de forma assistida, mantendo-o visível ao usuário.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Avaliação de desempenho da solução para diferentes volumes e modelos de dados
        destino.
\end{enumerate}
...
As etapas seguintes trataram da implementação da solução proposta, ``Polyglot Import
CSV'', e dos testes da mesma com diferentes volumes de dados CSV e importações simples e
complexas para diferentes modelos de dados destino. A última etapa consistiu na avaliação
de desempenho da ferramenta, cujos resultados --- \textit{throughput} por SGBD, ganho das
estratégias de escrita otimizadas e economia de memória do modo de execução em fluxo ---
são apresentados no Capítulo~\ref{cap:avaliacao}.

\section{Estrutura do Trabalho}

Este trabalho está estruturado em sete capítulos principais. O primeiro aborda os
objetivos, a metodologia e o contexto que levaram à elaboração deste trabalho. O capítulo 2
revisa os fundamentos teóricos e o capítulo 3 apresenta os trabalhos relacionados. O
capítulo 4 descreve a proposta da ferramenta \textit{Polyglot Import CSV} --- o formato de
configuração, o algoritmo de importação, a observabilidade e os modelos de execução. O
capítulo 5 apresenta a avaliação de desempenho da ferramenta, com resultados reais. O
capítulo 6 discute as atividades futuras e o capítulo 7 traz as considerações finais.
