% TODO(TCC2): atualizar listagens de filters/sources na reescrita do capítulo
% TODO(TCC2): apendiceconfig.tex promete "filtros" que o import_config.json atual nao contem — ajustar o texto do apendice ou reintroduzir um exemplo de filtro na config
% TODO(TCC2): images/figure10-config-pipeline.mmd cita load_csv_with_inference(), removida do codigo (pipeline v2 usa sources.py + mapping_resolver.py + casting.py) — redesenhar a figura na reescrita
...
\textbf{Raízes simétricas.} Ambos os schemas partilham a mesma raiz: um bloco
opcional por SGBD (\texttt{postgres}, \texttt{mongodb}, \texttt{cassandra},
\texttt{redis}, \texttt{neo4j}), com \texttt{additionalProperties: false}
rejeitando chaves desconhecidas. A diferença está no conteúdo de cada bloco: no
arquivo de importação ele descreve \textit{o que} mapear; no de conexão,
\textit{onde} gravar.
...
bloco opcional por SGBD. A cláusula \texttt{additionalProperties: false} na raiz
